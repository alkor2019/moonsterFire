...
let difficulty=5;
var k ={
    ArrowUp:0,ArrowLeft:0,ArrowRight:0
}
...
function handleStart(e) {
    ...
    for (let i = 0; i < touches.length; i++) {
        var touch = touches[i];
        checkButton(touch.pageX,touch.pageY)
...
createCanvas.addEventListener("click", handleClick,false);

function handleClick(e) {
    checkButton(e.clientX,e.clientY);
    e.preventDefault();
}
...
function checkButton(x,y) {
    if (x > ((createCanvas.width /2) -25) && x < ((createCanvas.width /2) +25)
    && y>((createCanvas.height/3)+50) &&
    y<((createCanvas.height/3)+100)&& !playing 
 ) {
     window.location.reload();
 }

 if (x > 20 && x< 90
 &&y>(createCanvas.height-90) &&
 y<(createCanvas.height-20)&& 
 playing 
) {
   k.ArrowLeft =1;
 }
 if (x > 110 && x < 180
     ...
     if (x> (createCanvas.width-90) && x < (createCanvas.width -20)
         && y>(createCanvas.height-90) &&
         y<(createCanvas.height-20)&& 
         playing 
      ) {
